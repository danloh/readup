...
class NativeBridgePlugin: Plugin {
  ...
  @objc public override func load(webview: WKWebView) {
    ...
    // covers annotation toolbar. See ContextMenuSuppressor.
    ...
    webview.configuration.userContentController.add(
      ShareBridgeMessageHandler(owner: self), name: "readupShareBridge")
  ...
  /// `window.webkit.messageHandlers.readupShareBridge.postMessage({type:'ready'})`
  ...
  /// Bridge between the Share Extension (separate process) and
  /// the host app's JS, via the App Group container at
  /// ` Two directions on every activation:
  ///
  ///   1. Groups (host → extension). Read the current library group list
  ///      from JS (`window.__readupGetGroups`) and persist it so the
  ...
  ///      (`window.__readupOnShareExtensionPending`), then clear the
  ...
  private func syncShareExtensionState() {
    DispatchQueue.main.async { [weak self] in
      ...
      webView.evaluateJavaScript(
        "(window.__readupGetGroups && window.__readupGetGroups()) || null"
      ) { result, _ in
      ...
      let script =
        "(window.__readupOnShareExtensionPending && window.__readupOnShareExtensionPending(\(jsonString))) === true"
  ...
  @objc public func auth_with_safari(_ invoke: Invoke) throws {
    ...
    authSession = ASWebAuthenticationSession(url: authUrl, callbackURLScheme: "readup") {
    ...
    do {
      ...
      if let volumeKeys = args.volumeKeys {
        if volumeKeys {
          ...
          // TTSAudioSessionClaimed/Released observers must survive the
...
enum FolderBookmarkStore {
  ...
  private static let keyPrefix = "readup.folderBookmark."
...
private final class ShareBridgeMessageHandler: NSObject, WKScriptMessageHandler {
  ...
  func userContentController(
    ...
    guard message.name == "readupShareBridge" else { return }
