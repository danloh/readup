...
//
// - `env_disable`: READUP_DISABLE_UPDATER is set (explicit opt-out).
...
fn updater_disabled() -> bool {
    let env_disable = std::env::var("READUP_DISABLE_UPDATER").is_ok();
    #[cfg(target_os = "linux")]
    {
        let is_flatpak =
...
pub fn run() {
    ...
    builder.setup(|#[allow(unused_variables)] app| {
        ...
        // (Linux deb/rpm/pacman and Flatpak) and when READUP_DISABLE_UPDATER
        ...
        let init_script = format!(
            r#"
                window.__READUP_APP_RUN_ID__ = "{app_run_id}";
                if ({is_eink}) window.__READUP_IS_EINK = true;
                if ({cli_access}) window.__READUP_CLI_ACCESS = true;
                if ({is_appimage}) window.__READUP_IS_APPIMAGE = true;
                if ({updater_disabled}) window.__READUP_UPDATER_DISABLED = true;
...
mod tests {
    ...
    fn env_opt_out_disables_on_any_desktop() {
        // READUP_DISABLE_UPDATER is an explicit opt-out on every desktop OS.
