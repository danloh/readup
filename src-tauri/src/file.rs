...
mod tests {
    ...
    fn app_storage_does_not_authorize_brand_names_in_foreign_paths() {
        let roots = [PathBuf::from(
            "/Users/victim/Library/Application Support/cc.readup",
        )];
        for path in [
            "/Users/victim/Library/LaunchAgents/Readup-agent.plist",
            "/Users/victim/Documents/cc.readup-secrets.txt",
        ] {
            assert!(!is_within_app_storage(Path::new(path), &roots));
        }
        assert!(is_within_app_storage(
            Path::new("/Users/victim/Library/Application Support/cc.readup/Readup/Books/book.epub"),
            &roots,
        ));
    }
    ...
    fn resolves_symlinks_before_authorizing_new_downloads() {
        let temp = std::env::temp_dir().join(format!("readup-path-scope-{}", std::process::id()));
