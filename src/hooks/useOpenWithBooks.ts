...
export function useOpenWithBooks() {
  ...
  useEffect(() => {
    ...
    // For Android "Share to Readup" intent
    let unlistenSharedIntent: Promise<PluginListener> | null = null;
    // register/unregister plugin listeniner on iOS might cause app freeze
    // so we only register it on Android for now to support "Shared to Readup" feature
