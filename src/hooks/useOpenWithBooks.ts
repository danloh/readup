...
/**
 * Handle "Open with Reader" file imports. Consumes the `app-incoming-url`
 ...
 *
 *   `'VIEW'` (Android only — user picked Reader from the system "Open with"
 ...
 *   Reader" share is meant to honour.
 */
  ...
  useEffect(() => {
    ...
    const normalizeUrls = (urls: string[]): string[] => {
      ...
      for (let url of urls) {
        ...
        if (!/^(https?:|data:|blob:|readup:)/i.test(url)) {
          filePaths.push(url);
        }
      }
      return filePaths;
