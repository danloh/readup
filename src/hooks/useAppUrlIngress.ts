...
interface SharedIntentPayload {
  urls: string[];
  /**
   * Android-only. Distinguishes "Open with Reader" (`VIEW` — the user
   * tapped a file in their file browser and chose Reader) from "Send to
   * Reader" (`SEND` / `SEND_MULTIPLE` — share-sheet capture). We forward
 ...
 *   - `shared-intent` plugin   — Android "Share to Reader" intent
...
export function useAppUrlIngress() {
  ...
  useEffect(() => {
    ...
    // side, so any "Open with Reader" intent that arrived AFTER cold
