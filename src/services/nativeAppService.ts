...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'readup') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override hasUpdater =
    ...
    !window.__READUP_UPDATER_DISABLED;
  ...
  override async init() {
    ...
    // Ask Rust whether the in-app updater must stay hidden (READUP_DISABLE_UPDATER,
    // Flatpak, or a Linux deb/rpm/pacman install that Tauri can't self-update). The
    // command is the reliable source of truth; the `__READUP_UPDATER_DISABLED`
