...
import { SCROLL_WRAPPER_CLASS, SCROLL_WRAPPER_FIT_CLASS } from '@/utils/scrollable';
...
export const transformStylesheet = (
  ...
  css = css.replace(/url\(\s*(?:"[^"]*"|'[^']*'|[^)]*)\s*\)/gi, (url) => {
    urlTokens.push(url);
    return `READUP_URL_${urlTokens.length - 1}_PLACEHOLDER`;
  });
  css = css.replace(
    ...
    (match, prop: string, value: string) => {
      ...
      const masked = value.replace(/[\w-]*\([^)]*\)/g, (fn) => {
        fns.push(fn);
        return `READUP_FN_${fns.length - 1}_PLACEHOLDER`;
      });
      const rewritten = masked.replace(/\bfixed\b/gi, 'scroll');
      return prop + rewritten.replace(/READUP_FN_(\d+)_PLACEHOLDER/g, (_, i) => fns[+i]!);
    },
  );
  css = css.replace(/READUP_URL_(\d+)_PLACEHOLDER/g, (_, i) => urlTokens[+i]!);
  ...
  css = css.replace(/"[^"\n]*"|'[^'\n]*'/g, (value) => {
    quoted.push(value);
    return `READUP_STR_${quoted.length - 1}_PLACEHOLDER`;
  });
  ...
  css = css.replace(/READUP_STR_(\d+)_PLACEHOLDER/g, (_, i) => quoted[+i]!);
  ...
  css = css
    ...
    // drops the whole declaration and the book loses its fonts (#5277).
    // The placeholders are underscore-wrapped so `\b` never matches inside them.
    .replace(/var\(\s*--(sans-serif|serif|monospace)\s*,\s*\1\s*\)/gi, 'READUP_GF_$1_PLACEHOLDER')
    .replace(/(font-family\s*:[^;]*?)\bsans-serif\b/gi, '$1READUP_SS_PLACEHOLDER')
    .replace(/(font-family\s*:[^;]*?)\bserif\b(?!-)/gi, '$1var(--serif, serif)')
    .replace(/READUP_SS_PLACEHOLDER/g, 'var(--sans-serif, sans-serif)')
    .replace(/(font-family\s*:[^;]*?)\bmonospace\b/gi, '$1var(--monospace, monospace)')
    .replace(/READUP_GF_(sans-serif|serif|monospace)_PLACEHOLDER/gi, 'var(--$1, $1)')
