...
import { FeedResponse, webFetchArticle } from '@/app/feed/components/dataAgent';
...
export async function GET(request: NextRequest) {
  ...
  if (!url) {
    return NextResponse.json(
      { error: 'Missing URL parameter. Usage: /api/feed/article?url=YOUR_URL' },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await webFetchArticle(url);
