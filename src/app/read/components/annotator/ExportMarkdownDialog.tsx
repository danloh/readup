  ...
  const markdownPreview = useMemo(() => {
    ...
    if (exportConfig.useCustomTemplate) {
      ...
      const templateData = {
        ...
        chapters: sortedGroups.map((group) => ({
          ...
          annotations: group.booknotes.map((note) => ({
            ...
            bookHash,
            link: buildShareUrl(
              { bookHash, noteId: note.id, did: user?.did, cfi: note.cfi },
              exportConfig.linkType,
            ),
            webLink: buildShareWebUrl(
              { bookHash, noteId: note.id, did: user?.did, cfi: note.cfi }
            ),
            appLink: buildShareAppUrl(
              { bookHash, noteId: note.id, did: user?.did, cfi: note.cfi }
            ),
            text: note.text || '',
            note: note.note || '',
      ...
      if (exportConfig.includeDate) {
        lines.push(`**${_('Exported from Readup')}**: ${new Date().toISOString().slice(0, 10)}`);
        lines.push('');
      }
...
const getDefaultTemplate = (_: any) => 
`## {{ title }}
**${_('Author')}**: {{ author }}

**${_('Exported from Readup')}**: {{ exportDate | date('%Y-%m-%d') }}
