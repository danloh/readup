...
const TTS_HIGHLIGHT_NAME = 'readup-tts-paragraph';
...
const ANNOTATION_HIGHLIGHT_PREFIX = 'readup-annotation-';
...
const ParagraphOverlay: React.FC<ParagraphOverlayProps> = ({
  ...
  const ttsHighlightCss = useMemo(
    () => buildTtsHighlightCssText({
      style: 'highlight', color: viewSettings?.ttsHighlightColor || ''
    }),
  ...
  const syncNativeSelectionMenu = useCallback(
    (suppressed: boolean) => {
      ...
      // setSelectionSuppressed({ target: 'menu', suppressed }).catch(() => {}); // TODO: 5430
    },
