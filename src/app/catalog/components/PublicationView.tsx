...
import { 
  classifyAcquisitionLink, getFormatName, getFormatTier, pickPreferredLink 
...
export function PublicationView({
  ...
  // sanitizes either source (issue #4749).
          ...
          {(acquisitionLinks.length > 0 || streamLinks.length > 0) && (
            <div className='flex flex-wrap items-center gap-2'>
              {acquisitionLinks.map(({ rel, links }) => {
                ...
                // Drop formats Reader cannot import — unless that would leave
              ...
              {publication.metadata.subject.map((subject, index: number) => {
                ...
                const inner = (
                  <>
                    ...
                    <div className='truncate' title={tag}>{tag}</div>
                  </>
                );
                return href ? (
                  <button
