 ...
 * (issue #5183). Restore the commas for display.
 */
 ...
 * `Url` templates.
 *
 * Some catalogs (a Nextcloud-hosted Calibre2OPDS instance in issue
...
export const resolveURL = (url: string, relativeTo: string): string => {
  ...
  try {
    if (relativeTo.includes(':')) {
      ...
      // `http://` links to themselves. bookserver.mek.oszk.hu (issue
