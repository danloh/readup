...
describe('opdsReq', () => {
  let needsProxy: typeof import('@/app/catalog/utils/opdsReq').needsProxy;
  let getProxiedURL: typeof import('@/app/catalog/utils/opdsReq').getProxiedURL;
  ...
  beforeEach(async () => {
    ...
    const opdsReq = await import('@/app/catalog/utils/opdsReq');
  ...
  describe('fetchWithAuth', () => {
    let fetchWithAuth: typeof import('@/app/catalog/utils/opdsReq').fetchWithAuth;
    ...
    it('passes preemptive auth through the proxy URL when useProxy is true', async () => {
      ...
      const auth = new URL(proxyUrl, 'https://readup.cc').searchParams.get('auth');
    ...
    it('recovers from the preemptive-Basic 400 through the proxy as well', async () => {
      ...
      expect(new URL(bareUrl, 'https://readup.cc').searchParams.get('auth')).toBeNull();
      const digestUrl = fetchMock.mock.calls[2]![0] as string;
      expect(new URL(digestUrl, 'https://readup.cc').searchParams.get('auth')).toMatch(
        /^Digest /,
      );
      expect(res.status).toBe(200);
    });
  ...
  describe('Origin header suppression on Tauri (#5698)', () => {
    ...
    let fetchWithAuth: typeof import('@/app/catalog/utils/opdsReq').fetchWithAuth;
    let probeAuth: typeof import('@/app/catalog/utils/opdsReq').probeAuth;
