...
const loadModule = async (runId?: string) => {
  vi.resetModules();
  if (runId === undefined) {
    delete window.__READUP_APP_RUN_ID__;
  } else {
    window.__READUP_APP_RUN_ID__ = runId;
  }
  return import('@/utils/deeplinkConsume');
};

const URL_A = 'readup://book/hashA';
const URL_B = 'readup://book/hashB';
...
describe('markLaunchUrl', () => {
  ...
  it('bounds the per-scope set', async () => {
    const { markLaunchUrl } = await loadModule('run-1');
    for (let i = 0; i < 40; i++) markLaunchUrl('book', `readup://book/h${i}`);
    ...
    expect(stored.urls[0]).toBe('readup://book/h24');
    expect(markLaunchUrl('book', 'readup://book/h39')).toBe(false);
  });
