...
vi.mock('@/services/environment', () => ({
  ...
  getBaseUrl: () => 'https://readup.cc',
  getNodeBaseUrl: () => 'https://readup.cc',
  isWebDevMode: () => true,
}));
...
vi.mock('@/app/catalog/utils/opdsReq', () => ({
  fetchWithAuth: vi.fn(),
}));
...
} from '@/app/catalog/utils/opdsUtils';
import type { OPDSBaseLink, OPDSCatalog, OPDSSearch } from '@/types/opds';
import { fetchWithAuth } from '@/app/catalog/utils/opdsReq';
...
describe('opdsUtils', () => {
  ...
  describe('formatContributorName', () => {
    ...
    // Calibre-Web OPDS feeds serve that raw form (issue #5183).
  ...
  describe('normalizeOpenSearchTemplates', () => {
    ...
    // issue #5500: a Nextcloud-hosted Calibre2OPDS catalog publishes its
  ...
  describe('resolveURL', () => {
    ...
    // bookserver.mek.oszk.hu (issue #5300) serves its catalog over HTTPS
