...
import { useMetadataEdit } from '@/app/library/components/metadata';
...
describe('useMetadataEdit numeric series fields', () => {
  it('stores the series index as a number', () => {
    const { result } = renderHook(() => useMetadataEdit(metadata));
  ...
  it('clears the index when the input is emptied', () => {
    const { result } = renderHook(() => useMetadataEdit(indexedMetadata));
