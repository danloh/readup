...
describe('fixed-layout containerPosition', () => {
  ...
  it('exposes a writable containerPosition for scrolled fixed-layout autoscroll', () => {
    ...
    // fixed-EPUB books in scrolled mode; the setter must exist.
    expect(typeof descriptor?.set).toBe('function');
  });

  it('observes scroll-direction for horizontal scroll mode (#4995)', () => {
