...
// Regression test for #4727: in fixed-layout scroll mode, a wheel tick
...
describe('fixed-layout scroll mode wheel handling (#4727)', () => {
  it('does not double-scroll the host when a wheel lands on a page', async () => {
    ...
    // (CI flake). The buggy scrollBy is synchronous, so a synchronous
