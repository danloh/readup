...
import {
  ...
  READUP_WEB_BASE_URL,
  READUP_UPDATER_FILE,
  READUP_CHANGELOG_FILE,
  READUP_OPDS_USER_AGENT,
  MAX_ZOOM_LEVEL,
  MIN_ZOOM_LEVEL,
  ZOOM_STEP,
...
describe('services/constants', () => {
  ...
  describe('DEFAULT_BOOK_STYLE', () => {
    ...
    it('has boolean style flags', () => {
      ...
      expect(typeof DEFAULT_BOOK_STYLE.invertImgColor).toBe('boolean');
  ...
  describe('URL constants', () => {
    ...
    it('DOWNLOAD_READUP_URL is a valid URL', () => {
      expect(DOWNLOAD_READUP_URL).toMatch(/^https:\/\//);
    });

    it('READUP_WEB_BASE_URL is a valid URL', () => {
      expect(READUP_WEB_BASE_URL).toMatch(/^https:\/\//);
    });

    it('READUP_UPDATER_FILE is a URL ending with .json', () => {
      expect(READUP_UPDATER_FILE).toMatch(/^https:\/\//);
      expect(READUP_UPDATER_FILE).toMatch(/\.json$/);
    });

    it('READUP_CHANGELOG_FILE is a URL ending with .json', () => {
      expect(READUP_CHANGELOG_FILE).toMatch(/^https:\/\//);
      expect(READUP_CHANGELOG_FILE).toMatch(/\.json$/);
    });

    it('READUP_OPDS_USER_AGENT is a non-empty string', () => {
      expect(typeof READUP_OPDS_USER_AGENT).toBe('string');
      expect(READUP_OPDS_USER_AGENT.length).toBeGreaterThan(0);
    });
  });
