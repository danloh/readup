...
describe('punctuationTransformer', () => {
  ...
  test('returns empty string unchanged', async () => {
    const result = await punctuationTransformer.transform(
      makeCtx({
        content: '',
        viewSettings: { vertical: true } as ViewSettings,
      }),
    );
    expect(result).toBe('');
  });
...
describe('styleTransformer', () => {
  ...
  beforeEach(async () => {
    const styleMod = await import('@/styles/style');
...
describe('nbspTransformer', () => {
  let nbspTransformer: typeof import('@/app/read/transformers/nbsp').nbspTransformer;

  beforeEach(async () => {
    ({ nbspTransformer } = await import('@/app/read/transformers/nbsp'));
  });
