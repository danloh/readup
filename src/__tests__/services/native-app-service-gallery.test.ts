...
describe('NativeAppService.saveImageToGallery', () => {
  ...
  test('logs the native error when the MediaStore insert fails', async () => {
    ...
    saveImageToGalleryMock.mockResolvedValue({
      success: false,
      error: 'Failed to build unique file: /storage/emulated/0/Pictures/Readup image.png',
    });
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      'Failed to build unique file: /storage/emulated/0/Pictures/Readup image.png',
    );
  });
});
