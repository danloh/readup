...
import { READUP_UPDATER_FILE, READUP_CHANGELOG_FILE } from '@/services/constants';
...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `Readup_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `Readup_${data.version}_${arch}.AppImage`;
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(READUP_CHANGELOG_FILE);
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of Readup is available!')}
              </h2>
              <p className='mb-2'>
                {_('Readup {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=cc.readup'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in Readup")}
