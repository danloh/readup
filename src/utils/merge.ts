 ...
 * note based on `updatedAt` / `deletedAt`.
 *
 ...
 *
 * Scalars use a per-config `updatedAt` LWW; booknotes always merge via the
 * element-set CRDT regardless of which side won the scalar race. Null /
 * undefined remote fields are dropped before the spread/
 *
...
export const mergeBookConfig = (
  ...
  const remoteConfigUpdated = remote.updatedAt;
 ...
 * newer on EITHER clock — book metadata (`updatedAt`).
 */
